...
#define SHCAMLED_TORCH_PERMISSION	(S_IWUSR | S_IWGRP)
...
//				qpnp_flash_control_enable(false);
		...
		led_current = mA * 2;
		if(led_current > 25){
			led_current = led_current - 25;
		} else {
			led_current = 0;
		}
		if(mA > 200){
			led_current = (led_current * 0x50 + 0x9D) / (0x4F * 2);
		} else {
			led_current = (led_current * 0x50 + 0x1D) / (0x0F * 2);
		}
		if(led_current < 1){
			led_current = 1;
		}
...
#endif
	led_trigger_register_simple("torch0_trigger", &cam_torch_led_1_trigger);
