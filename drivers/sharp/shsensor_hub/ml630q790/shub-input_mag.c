/*
 *  shub-input_mag.c - Linux kernel modules for interface of ML630Q790
 *
...
static int shub_mag_log = 0;
module_param(shub_mag_log, int, 0600);
#define DBG_MAG_IO(msg, ...) {                      \
    if(shub_mag_log & 0x01)                         \
        printk("[shub][mag] " msg, ##__VA_ARGS__);  \
}
#define DBG_MAG_DATA(msg, ...) {                    \
    if(shub_mag_log & 0x02)                         \
        printk("[shub][mag] " msg, ##__VA_ARGS__);  \
}
#else
#define DBG_MAG_IO(msg, ...)
...
#define INDEX_TMNS         5
#define INDEX_SUM          6
#define INPUT_DEV_NAME "shub_mag"
#define INPUT_DEV_PHYS "shub_mag/input0"
#define MISC_DEV_NAME  "shub_io_mag"
#define SHUB_EVENTS_PER_PACKET ((LOGGING_RAM_SIZE/(DATA_SIZE_MAG+1))*2) /*+1 is ID size*/
#define SHUB_ACTIVE_SENSOR SHUB_ACTIVE_MAG
...
static long shub_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        case SHUBIO_MAG_ACTIVATE:
            ret = copy_from_user(&tmpval, argp, sizeof(tmpval));
            if (ret) {
                printk("error : shub_ioctl(cmd = SHUBIO_MAG_ACTIVATE)\n");
                return -EFAULT;
            }
            DBG_MAG_IO("ioctl(cmd = Set_Active) : val=%d\n", tmpval); // SHMDS_HUB_0701_01 add
        ...
        case SHUBIO_MAG_SET_FREQ:
            { 
                ret = copy_from_user(&tmpval, argp, sizeof(tmpval));
                if (ret) {
                    printk( "error : shub_ioctl(cmd = SHUBIO_MAG_SET_FREQ)\n" );
                    return -EFAULT;
                }
                DBG_MAG_IO("ioctl(cmd = Set_Delay) : delay=%d\n", tmpval); // SHMDS_HUB_0701_01 add
                mutex_lock(&shub_lock);
                delay = tmpval;
        ...
        case SHUBIO_MAG_SET_BATCH :
            {
                IoCtlBatchInfo param;
                uint64_t delayNs;
                ret = copy_from_user(&param, argp, sizeof(param));
                if (ret) {
                    printk( "error(copy_from_user) : shub_ioctl(cmd = SHUBIO_MAG_SET_BATCH )");
                    return -EFAULT;
                }
                DBG_MAG_IO("ioctl(cmd = Set_Batch) : flg=%d, Period=%lld, Latency=%lld\n", param.m_Flasg, param.m_PeriodNs, param.m_Latency); // SHMDS_HUB_0701_01 add
        ...
        case SHUBIO_MAG_FLUSH :
            {
                DBG_MAG_IO("ioctl(cmd = Flush)\n"); // SHMDS_HUB_0701_01 add
                mutex_lock(&shub_lock);
                if(power_state != 0){
                    shub_logging_flush();
                    setMaxBatchReportLatency(SHUB_ACTIVE_SENSOR, batch_param.m_Latency);
...
//                  input_event(shub_idev, EV_SYN, SYN_REPORT, 2);
                    ...
                    input_event(shub_idev, EV_SYN, SYN_REPORT, (SHUB_INPUT_META_DATA | SHUB_INPUT_MAG));
        ...
        case SHUBIO_MAG_SET_OFFSET:
            {
                ...
                if (ret) {
                    printk( "error(copy_from_user) : shub_ioctl(cmd = SHUBIO_MAG_SET_OFFSET)\n" );
                    return -EFAULT;
                }
                DBG_MAG_IO("ioctl(cmd = Set_Offset) : x=%x, y=%x, z=%x\n", ioc.m_iOffset[0], ioc.m_iOffset[1], ioc.m_iOffset[2]); // SHMDS_HUB_0701_01 add
                shub_set_mag_offset(ioc.m_iOffset);
            }
            break;

        case SHUBIO_MAG_GET_OFFSET:
            {
                IoCtlSetOffset ioc;
                shub_get_mag_offset(ioc.m_iOffset);
                DBG_MAG_IO("ioctl(cmd = Get_Offset) : x=%x, y=%x, z=%x\n", ioc.m_iOffset[0], ioc.m_iOffset[1], ioc.m_iOffset[2]); // SHMDS_HUB_0701_01 add
                ret = copy_to_user(argp, &ioc, sizeof(IoCtlSetOffset));
                if (ret) {
                    printk( "error(copy_from_user) : shub_ioctl(cmd = SHUBIO_MAG_GET_OFFSET )\n" );
                    return -EFAULT;
                }
            }
            break;

        case SHUBIO_MAG_SET_POSITION:
            {
                IoCtlParam ioc;
                memset(&ioc , 0x00, sizeof(ioc));
                ret = copy_from_user(&ioc,argp,sizeof(ioc));
                if (ret) {
                    printk( "error(copy_from_user) : shub_ioctl(cmd = SHUBIO_MAG_SET_POSITION)\n" );
                    return -EFAULT;
                }
                DBG_MAG_IO("ioctl(cmd = Set_Position) : type=%d\n", ioc.m_iType); // SHMDS_HUB_0701_01 add
                shub_set_mag_position(ioc.m_iType);
            }
            break;

        case SHUBIO_MAG_CALIBRATE_AXIS_ITNERFERANCE:
            {
                IoCtlParam ioc;
                memset(&ioc , 0x00, sizeof(ioc));
                ret = copy_from_user(&ioc,argp,sizeof(ioc));
                ...
                if (ret) {
                    printk( "error(copy_from_user) : shub_ioctl(cmd = SHUBIO_MAG_CALIBRATE_AXIS_ITNERFERANCE)\n" );
                    return -EFAULT;
                }
                DBG_MAG_IO("ioctl(cmd = Calibrate_Axis) : type=%d\n", ioc.m_iType); // SHMDS_HUB_0701_01 add
                if(ioc.m_iType == 0){
                    shub_cal_mag_axis_interfrence(ioc.m_iParam);
                }else{
                    shub_cal_mag_axis_interfrence(NULL);
                }
            }
            break;
...
/* SHMDS_HUB_0311_02 mod S */
                ...
                shub_get_sensors_data(SHUB_ACTIVE_MAGUNC, xyz_mag_uc);
...
void shub_input_report_mag(int32_t *data)
{
    ...
    data[INDEX_X] = shub_adjust_value(MAG_MIN, MAG_MAX,data[INDEX_X]);
    data[INDEX_Y] = shub_adjust_value(MAG_MIN, MAG_MAX,data[INDEX_Y]);
    data[INDEX_Z] = shub_adjust_value(MAG_MIN, MAG_MAX,data[INDEX_Z]);
...
// SHMDS_HUB_0701_01 add E
    ...
    input_event(shub_idev, EV_SYN, SYN_REPORT, SHUB_INPUT_MAG);
#else
...
static void shub_set_abs_params(void)
{
    ...
    input_set_abs_params(shub_idev, ABS_RX, MAG_MIN, MAG_MAX, 0, 0);
    input_set_abs_params(shub_idev, ABS_RY, MAG_MIN, MAG_MAX, 0, 0);
    input_set_abs_params(shub_idev, ABS_RZ, MAG_MIN, MAG_MAX, 0, 0);
...
static int32_t __init shub_init(void)
{
    ...
    if(!shub_connect_check()){
        printk(KERN_INFO "shub_mag Connect Error!!\n");
        ret = -ENODEV;
        goto out_driver;
    }
...
#if 1  // SHMDS_HUB_0601_01 mod S
    shub_idev = shub_com_allocate_device(SHUB_INPUT_MAG, &pdev->dev);
    if (!shub_idev) {
        ret = -ENOMEM;
        goto out_device;
    }
...
#if 1  // SHMDS_HUB_0601_01 mod S
    shub_com_unregister_device(SHUB_INPUT_MAG);
#else
...
MODULE_DESCRIPTION("SensorHub Input Device (Magnetic)");
