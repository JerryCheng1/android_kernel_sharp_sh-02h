/*
 *  shub-input_linear_acc.c - Linux kernel modules for interface of ML630Q790
 *
...
static int shub_linearacc_log = 0;
module_param(shub_linearacc_log, int, 0600);
#define DBG_LINEARACC_IO(msg, ...) {                      \
    if(shub_linearacc_log & 0x01)                         \
        printk("[shub][linearacc] " msg, ##__VA_ARGS__);  \
}
#define DBG_LINEARACC_DATA(msg, ...) {                    \
    if(shub_linearacc_log & 0x02)                         \
        printk("[shub][linearacc] " msg, ##__VA_ARGS__);  \
}
#else
#define DBG_LINEARACC_IO(msg, ...)
...
#define INDEX_TMNS     4
#define INDEX_SUM      5
#define INPUT_DEV_NAME "shub_linearacc"
#define INPUT_DEV_PHYS "shub_linearacc/input0"
#define MISC_DEV_NAME  "shub_io_linearacc"
#define SHUB_EVENTS_PER_PACKET ((LOGGING_RAM_SIZE/(DATA_SIZE_LINEARACC+1))*2) /*+1 is ID size*/
#define SHUB_ACTIVE_SENSOR SHUB_ACTIVE_LACC
...
static long shub_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        case SHUBIO_LINEARACC_ACTIVATE:
            ret = copy_from_user(&tmpval, argp, sizeof(tmpval));
            if (ret) {
                printk("error : shub_ioctl(cmd = SHUBIO_LINEARACC_ACTIVATE)\n");
                return -EFAULT;
            }
            DBG_LINEARACC_IO("ioctl(cmd = Set_Active) : val=%d\n", tmpval); // SHMDS_HUB_0701_01 add
        ...
        case SHUBIO_LINEARACC_SET_FREQ:
            {
                ret = copy_from_user(&tmpval, argp, sizeof(tmpval));
                if (ret) {
                    printk( "error : shub_ioctl(cmd = SHUBIO_LINEARACC_SET_FREQ)\n" );
                    return -EFAULT;
                }
                DBG_LINEARACC_IO("ioctl(cmd = Set_Delay) : delay=%d\n", tmpval); // SHMDS_HUB_0701_01 add
                mutex_lock(&shub_lock);
                delay = tmpval;
        ...
        case SHUBIO_LINEARACC_SET_BATCH :
            {
                IoCtlBatchInfo param;
                uint64_t delayNs;
                ret = copy_from_user(&param, argp, sizeof(param));
                if (ret) {
                    printk( "error(copy_from_user) : shub_ioctl(cmd = SHUBIO_LINEARACC_SET_BATCH)\n" );
                    return -EFAULT;
                }
                DBG_LINEARACC_IO("ioctl(cmd = Set_Batch) : flg=%d, Period=%lld, Latency=%lld\n", param.m_Flasg, param.m_PeriodNs, param.m_Latency); // SHMDS_HUB_0701_01 add
        ...
        case SHUBIO_LINEARACC_FLUSH :
            {
                DBG_LINEARACC_IO("ioctl(cmd = Flush)\n"); // SHMDS_HUB_0701_01 add
                mutex_lock(&shub_lock);
                if(power_state != 0){
                    shub_logging_flush();
                    setMaxBatchReportLatency(SHUB_ACTIVE_SENSOR, batch_param.m_Latency);
...
//                  input_event(shub_idev, EV_SYN, SYN_REPORT, 2);
                    ...
                    input_event(shub_idev, EV_SYN, SYN_REPORT, (SHUB_INPUT_META_DATA | SHUB_INPUT_LINEARACC));
...
void shub_input_report_linear(int32_t *data)
{
    ...
    data[INDEX_X] = shub_adjust_value(LINEACC_MIN, LINEACC_MAX,data[INDEX_X]);
    data[INDEX_Y] = shub_adjust_value(LINEACC_MIN, LINEACC_MAX,data[INDEX_Y]);
    data[INDEX_Z] = shub_adjust_value(LINEACC_MIN, LINEACC_MAX,data[INDEX_Z]);

// SHMDS_HUB_0701_01 add S
    DBG_LINEARACC_DATA("data X=%d, Y=%d, Z=%d, t(s)=%d, t(ns)=%d\n", data[INDEX_X],data[INDEX_Y],data[INDEX_Z],data[INDEX_TM],data[INDEX_TMNS]);
// SHMDS_HUB_0701_01 add E
    ...
    shub_input_sync_init(shub_idev); /* SHMDS_HUB_0602_01 mod */
    input_event(shub_idev, EV_SYN, SYN_REPORT, SHUB_INPUT_LINEARACC);
#else
...
static void shub_set_abs_params(void)
{
    ...
    input_set_abs_params(shub_idev, ABS_X, LINEACC_MIN, LINEACC_MAX, 0, 0);
    input_set_abs_params(shub_idev, ABS_Y, LINEACC_MIN, LINEACC_MAX, 0, 0);
    input_set_abs_params(shub_idev, ABS_Z, LINEACC_MIN, LINEACC_MAX, 0, 0);
...
static int32_t __init shub_init(void)
{
    ...
    if(!shub_connect_check()){
        printk(KERN_INFO "shub_lineacc Connect Error!!\n");
        ret = -ENODEV;
        goto out_driver;
    }
...
#if 1  // SHMDS_HUB_0601_01 mod S
    shub_idev = shub_com_allocate_device(SHUB_INPUT_LINEARACC, &pdev->dev);
    if (!shub_idev) {
        ret = -ENOMEM;
        goto out_device;
    }
...
#if 1  // SHMDS_HUB_0601_01 mod S
    shub_com_unregister_device(SHUB_INPUT_LINEARACC);
#else
...
MODULE_DESCRIPTION("SensorHub Input Device (LinearAcc)");
