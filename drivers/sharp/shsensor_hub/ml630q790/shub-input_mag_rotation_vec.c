/*
 *  shub-input_mag_rotation_vec.c - Linux kernel modules for interface of ML630Q790
 *
...
static int shub_georv_log = 0;
module_param(shub_georv_log, int, 0600);
#define DBG_GEORV_IO(msg, ...) {                      \
    if(shub_georv_log & 0x01)                         \
        printk("[shub][georv] " msg, ##__VA_ARGS__);  \
}
#define DBG_GEORV_DATA(msg, ...) {                    \
    if(shub_georv_log & 0x02)                         \
        printk("[shub][georv] " msg, ##__VA_ARGS__);  \
}
#else
#define DBG_GEORV_IO(msg, ...)
...
#define INDEX_TMNS         6
#define INDEX_SUM          7
#define INPUT_DEV_NAME "shub_georv"
#define INPUT_DEV_PHYS "shub_georv/input0"
#define MISC_DEV_NAME  "shub_io_georv"
#define SHUB_EVENTS_PER_PACKET ((LOGGING_RAM_SIZE/(DATA_SIZE_GEORV+1))*2) /*+1 is ID size*/
#define SHUB_ACTIVE_SENSOR SHUB_ACTIVE_RV_NONGYRO 
...
static long shub_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        case SHUBIO_GEORV_ACTIVATE:
            ret = copy_from_user(&tmpval, argp, sizeof(tmpval));
            if (ret) {
                printk("error : shub_ioctl(cmd = SHUBIO_GEORV_ACTIVATE)\n");
                return -EFAULT;
            }
            DBG_GEORV_IO("ioctl(cmd = Set_Active) : val=%d\n", tmpval); // SHMDS_HUB_0701_01 add
        ...
        case SHUBIO_GEORV_SET_FREQ:
            {
                ret = copy_from_user(&tmpval, argp, sizeof(tmpval));
                if (ret) {
                    printk( "error : shub_ioctl(cmd = SHUBIO_GEORV_SET_FREQ)\n" );
                    return -EFAULT;
                }
                DBG_GEORV_IO("ioctl(cmd = Set_Delay) : delay=%d\n", tmpval); // SHMDS_HUB_0701_01 add
                mutex_lock(&shub_lock);
                delay = tmpval;
        ...
        case SHUBIO_GEORV_SET_BATCH :
            {
                IoCtlBatchInfo param;
                uint64_t delayNs;
                ret = copy_from_user(&param, argp, sizeof(param));
                if (ret) {
                    printk( "error(copy_from_user) : shub_ioctl(cmd = SHUBIO_GEORV_SET_BATCH)\n" );
                    return -EFAULT;
                }
                DBG_GEORV_IO("ioctl(cmd = Set_Batch) : flg=%d, Period=%lld, Latency=%lld\n", param.m_Flasg, param.m_PeriodNs, param.m_Latency); // SHMDS_HUB_0701_01 add
        ...
        case SHUBIO_GEORV_FLUSH :
            {
                DBG_GEORV_IO("ioctl(cmd = Flush)\n"); // SHMDS_HUB_0701_01 add
                mutex_lock(&shub_lock);
                if(power_state != 0){
                    shub_logging_flush();
                    setMaxBatchReportLatency(SHUB_ACTIVE_SENSOR, batch_param.m_Latency);
...
//                  input_event(shub_idev, EV_SYN, SYN_REPORT, 2);
                    ...
                    input_event(shub_idev, EV_SYN, SYN_REPORT, (SHUB_INPUT_META_DATA | SHUB_INPUT_GEORV));
...
void shub_input_report_rot_mag(int32_t *data)
{
    ...
    data[INDEX_X] = shub_adjust_value(MAGROT_MIN, MAGROT_MAX,data[INDEX_X]);
    data[INDEX_Y] = shub_adjust_value(MAGROT_MIN, MAGROT_MAX,data[INDEX_Y]);
    data[INDEX_Z] = shub_adjust_value(MAGROT_MIN, MAGROT_MAX,data[INDEX_Z]);
    data[INDEX_S] = shub_adjust_value(MAGROT_MIN, MAGROT_MAX,data[INDEX_S]);

// SHMDS_HUB_0701_01 add S
    DBG_GEORV_DATA("data X=%d, Y=%d, Z=%d, S=%d, D=%d, t(s)=%d, t(ns)=%d\n", 
...
// SHMDS_HUB_0701_01 add E
    ...
    input_event(shub_idev, EV_SYN, SYN_REPORT, SHUB_INPUT_GEORV);
#else
...
static void shub_set_abs_params(void)
{
    ...
    input_set_abs_params(shub_idev, ABS_RX, MAGROT_MIN, MAGROT_MAX, 0, 0);
    input_set_abs_params(shub_idev, ABS_RY, MAGROT_MIN, MAGROT_MAX, 0, 0);
    input_set_abs_params(shub_idev, ABS_RZ, MAGROT_MIN, MAGROT_MAX, 0, 0);
    input_set_abs_params(shub_idev, ABS_HAT0Y, MAGROT_MIN, MAGROT_MAX, 0, 0);
...
static int32_t __init shub_init(void)
{
    ...
    if(!shub_connect_check()){
        printk(KERN_INFO "shub_magrot Connect Error!!\n");
        ret = -ENODEV;
        goto out_driver;
    }
...
#if 1  // SHMDS_HUB_0601_01 mod S
    shub_idev = shub_com_allocate_device(SHUB_INPUT_GEORV, &pdev->dev);
    if (!shub_idev) {
        ret = -ENOMEM;
        goto out_device;
    }
...
#if 1  // SHMDS_HUB_0601_01 mod S
    shub_com_unregister_device(SHUB_INPUT_GEORV);
#else
...
MODULE_DESCRIPTION("SensorHub Input Device (MagneticRotationVector)");
