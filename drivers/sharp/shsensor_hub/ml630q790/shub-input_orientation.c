/*
 *  shub-input_orientation.c - Linux kernel modules for interface of ML630Q790
 *
...
static int shub_ori_log = 0;
module_param(shub_ori_log, int, 0600);
#define DBG_ORI_IO(msg, ...) {                      \
    if(shub_ori_log & 0x01)                         \
        printk("[shub][ori] " msg, ##__VA_ARGS__);  \
}
#define DBG_ORI_DATA(msg, ...) {                    \
    if(shub_ori_log & 0x02)                         \
        printk("[shub][ori] " msg, ##__VA_ARGS__);  \
}
#else
#define DBG_ORI_IO(msg, ...)
...
#define INDEX_TMNS         5
#define INDEX_SUM          6
#define INPUT_DEV_NAME "shub_ori"
#define INPUT_DEV_PHYS "shub_ori/input0"
#define MISC_DEV_NAME  "shub_io_ori"
#define SHUB_EVENTS_PER_PACKET ((LOGGING_RAM_SIZE/(DATA_SIZE_ORI+1))*2) /*+1 is ID size*/
#define SHUB_ACTIVE_SENSOR SHUB_ACTIVE_ORI
...
static long shub_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        case SHUBIO_ORI_ACTIVATE:
            ret = copy_from_user(&tmpval, argp, sizeof(tmpval));
            if (ret) {
                printk("error : shub_ioctl(cmd = SHUBIO_ORI_ACTIVATE)\n");
                return -EFAULT;
            }
            DBG_ORI_IO("ioctl(cmd = Set_Active) : val=%d\n", tmpval); // SHMDS_HUB_0701_01 add
        ...
        case SHUBIO_ORI_SET_FREQ:
            {
                ret = copy_from_user(&tmpval, argp, sizeof(tmpval));
                if (ret) {
                    printk( "error : shub_ioctl(cmd = SHUBIO_ORI_SET_FREQ)\n" );
                    return -EFAULT;
                }
                DBG_ORI_IO("ioctl(cmd = Set_Delay) : delay=%d\n", tmpval); // SHMDS_HUB_0701_01 add
                mutex_lock(&shub_lock);
                delay = tmpval;
        ...
        case SHUBIO_ORI_SET_BATCH :
            {
                IoCtlBatchInfo param;
                uint64_t delayNs;
                ret = copy_from_user(&param, argp, sizeof(param));
                if (ret) {
                    printk( "error(copy_from_user) : shub_ioctl(cmd = SHUBIO_ORI_SET_BATCH\n" );
                    return -EFAULT;
                }
                DBG_ORI_IO("ioctl(cmd = Set_Batch) : flg=%d, Period=%lld, Latency=%lld\n", param.m_Flasg, param.m_PeriodNs, param.m_Latency); // SHMDS_HUB_0701_01 add
        ...
        case SHUBIO_ORI_FLUSH :
            {
                DBG_ORI_IO("ioctl(cmd = Flush)\n"); // SHMDS_HUB_0701_01 add
                mutex_lock(&shub_lock);
                if(power_state != 0){
                    shub_logging_flush();
                    setMaxBatchReportLatency(SHUB_ACTIVE_SENSOR, batch_param.m_Latency);
...
void shub_input_report_orien(int32_t *data)
{
    ...
    data[INDEX_X] = shub_adjust_value(ORI_MIN, ORI_MAX,data[INDEX_X]);
    data[INDEX_Y] = shub_adjust_value(ORI_MIN, ORI_MAX,data[INDEX_Y]);
    data[INDEX_Z] = shub_adjust_value(ORI_MIN, ORI_MAX,data[INDEX_Z]);

// SHMDS_HUB_0701_01 add S
    DBG_ORI_DATA("data X=%d, Y=%d, Z=%d, S=%d, t(s)=%d, t(ns)=%d\n", 
...
static void shub_set_abs_params(void)
{
    ...
    input_set_abs_params(shub_idev, ABS_RX, ORI_MIN, ORI_MAX, 0, 0);
    input_set_abs_params(shub_idev, ABS_RY, ORI_MIN, ORI_MAX, 0, 0);
    input_set_abs_params(shub_idev, ABS_RZ, ORI_MIN, ORI_MAX, 0, 0);
...
static int32_t __init shub_init(void)
{
    ...
    if(!shub_connect_check()){
        printk(KERN_INFO "shub_orient Connect Error!!\n");
        ret = -ENODEV;
        goto out_driver;
    }
...
MODULE_DESCRIPTION("SensorHub Input Device (Orientation)");
