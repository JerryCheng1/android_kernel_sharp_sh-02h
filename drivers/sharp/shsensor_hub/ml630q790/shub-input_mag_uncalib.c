/*
 *  shub-input_mag_uncalib.c - Linux kernel modules for interface of ML630Q790
 *
...
static int shub_magunc_log = 0;
module_param(shub_magunc_log, int, 0600);
#define DBG_MAGUNC_IO(msg, ...) {                      \
    if(shub_magunc_log & 0x01)                         \
        printk("[shub][magunc] " msg, ##__VA_ARGS__);  \
}
#define DBG_MAGUNC_DATA(msg, ...) {                    \
    if(shub_magunc_log & 0x02)                         \
        printk("[shub][magunc] " msg, ##__VA_ARGS__);  \
}
#else
#define DBG_MAGUNC_IO(msg, ...)
...
#define INDEX_Y            1
#define INDEX_Z            2
#define INDEX_X_BIAS       3
#define INDEX_Y_BIAS       4
#define INDEX_Z_BIAS       5
#define INDEX_ACC          6
#define INDEX_TM           7
#define INDEX_TMNS         8
#define INDEX_SUM          9
#define INPUT_DEV_NAME "shub_magunc"
#define INPUT_DEV_PHYS "shub_magunc/input0"
#define MISC_DEV_NAME  "shub_io_magunc"
#define SHUB_EVENTS_PER_PACKET ((LOGGING_RAM_SIZE/(DATA_SIZE_MAG+1))*2) /*+1 is ID size*/
#define SHUB_ACTIVE_SENSOR SHUB_ACTIVE_MAGUNC
...
static long shub_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        case SHUBIO_MAGUNCAL_ACTIVATE:
            ret = copy_from_user(&tmpval, argp, sizeof(tmpval));
            if (ret) {
                printk("error : shub_ioctl(cmd = SHUBIO_MAGUNCAL_ACTIVATE)\n");
                return -EFAULT;
            }
            DBG_MAGUNC_IO("ioctl(cmd = Set_Active) : val=%d\n", tmpval); // SHMDS_HUB_0701_01 add
        ...
        case SHUBIO_MAGUNCAL_SET_FREQ:
            {
                ret = copy_from_user(&tmpval, argp, sizeof(tmpval));
                if (ret) {
                    printk( "error : shub_ioctl(cmd = SHUBIO_MAGUNCAL_SET_FREQ)\n" );
                    return -EFAULT;
                }
                DBG_MAGUNC_IO("ioctl(cmd = Set_Delay) : delay=%d\n", tmpval); // SHMDS_HUB_0701_01 add
                mutex_lock(&shub_lock);
                delay = tmpval;
        ...
        case SHUBIO_MAGUNCAL_SET_BATCH :
            {
                IoCtlBatchInfo param;
                uint64_t delayNs;
                ret = copy_from_user(&param, argp, sizeof(param));
                if (ret) {
                    printk( "error(copy_from_user) : shub_ioctl(cmd = SHUBIO_MAGUNCAL_SET_BATCH)\n" );
                    return -EFAULT;
                }
                DBG_MAGUNC_IO("ioctl(cmd = Set_Batch) : flg=%d, Period=%lld, Latency=%lld\n", param.m_Flasg, param.m_PeriodNs, param.m_Latency); // SHMDS_HUB_0701_01 add
        ...
        case SHUBIO_MAGUNCAL_FLUSH :
            {
                DBG_MAGUNC_IO("ioctl(cmd = Flush)\n"); // SHMDS_HUB_0701_01 add
                mutex_lock(&shub_lock);
                if(power_state != 0){
                    shub_logging_flush();
                    setMaxBatchReportLatency(SHUB_ACTIVE_SENSOR, batch_param.m_Latency);
...
//                  input_event(shub_idev, EV_SYN, SYN_REPORT, 2);
                    ...
                    input_event(shub_idev, EV_SYN, SYN_REPORT, (SHUB_INPUT_META_DATA | SHUB_INPUT_MAGUNC));
...
/* SHMDS_HUB_0311_02 mod S */
                ...
                local_ts_ns = tmp_ts.tv_nsec + tmp_ts.tv_sec * 1000000000;
                if( (shub_local_ts_ns_old < (local_ts_ns - delay * 600000)) && 
				    ((shub_normal_last_ts.tv_nsec + shub_normal_last_ts.tv_sec * 1000000000) < (local_ts_ns - delay * 600000)) ){
...
void shub_input_report_mag_uncal(int32_t *data)
{
    ...
    data[INDEX_X] = shub_adjust_value(MAGUNC_MIN, MAGUNC_MAX,data[INDEX_X]);
    data[INDEX_Y] = shub_adjust_value(MAGUNC_MIN, MAGUNC_MAX,data[INDEX_Y]);
    data[INDEX_Z] = shub_adjust_value(MAGUNC_MIN, MAGUNC_MAX,data[INDEX_Z]);
    data[INDEX_X_BIAS] = shub_adjust_value(MAGUNC_MIN, MAGUNC_MAX,data[INDEX_X_BIAS]);
    data[INDEX_Y_BIAS] = shub_adjust_value(MAGUNC_MIN, MAGUNC_MAX,data[INDEX_Y_BIAS]);
    data[INDEX_Z_BIAS] = shub_adjust_value(MAGUNC_MIN, MAGUNC_MAX,data[INDEX_Z_BIAS]);

// SHMDS_HUB_0701_01 add S
    DBG_MAGUNC_DATA("data X=%d, Y=%d, Z=%d, x=%d, y=%d, z=%d, S=%d, t(s)=%d, t(ns)=%d\n",
                     data[INDEX_X],data[INDEX_Y],data[INDEX_Z],data[INDEX_X_BIAS],
                     data[INDEX_Y_BIAS],data[INDEX_Z_BIAS],data[INDEX_ACC],data[INDEX_TM],data[INDEX_TMNS]);
// SHMDS_HUB_0701_01 add E
    ...
    input_report_abs(shub_idev, ABS_HAT0Y, data[INDEX_ACC]);
...
static void shub_set_abs_params(void)
{
    ...
    input_set_abs_params(shub_idev, ABS_RX, MAGUNC_MIN, MAGUNC_MAX, 0, 0);
    input_set_abs_params(shub_idev, ABS_RY, MAGUNC_MIN, MAGUNC_MAX, 0, 0);
    input_set_abs_params(shub_idev, ABS_RZ, MAGUNC_MIN, MAGUNC_MAX, 0, 0);
    input_set_abs_params(shub_idev, ABS_X, MAGUNC_MIN, MAGUNC_MAX, 0, 0);
    input_set_abs_params(shub_idev, ABS_Y, MAGUNC_MIN, MAGUNC_MAX, 0, 0);
    input_set_abs_params(shub_idev, ABS_Z, MAGUNC_MIN, MAGUNC_MAX, 0, 0);
...
static int32_t __init shub_init(void)
{
    ...
    if(!shub_connect_check()){
        printk(KERN_INFO "shub_magunc Connect Error!!\n");
        ret = -ENODEV;
        goto out_driver;
    }
...
#if 1  // SHMDS_HUB_0601_01 mod S
    shub_idev = shub_com_allocate_device(SHUB_INPUT_MAGUNC, &pdev->dev);
    if (!shub_idev) {
        ret = -ENOMEM;
        goto out_device;
    }
...
#if 1  // SHMDS_HUB_0601_01 mod S
    shub_com_unregister_device(SHUB_INPUT_MAGUNC);
#else
...
MODULE_DESCRIPTION("SensorHub Input Device (MagneticUncal)");
