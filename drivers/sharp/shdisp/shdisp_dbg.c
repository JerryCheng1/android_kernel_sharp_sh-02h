...
/* ------------------------------------------------------------------------- */
    ...
    unsigned short shdisp_log_lv = SHDISP_LOG_LV_ERR | SHDISP_LOG_LV_WARN;
#else  /* CONFIG_ANDROID_ENGINEERING */
    unsigned short shdisp_log_lv = SHDISP_LOG_LV_ERR;
...
#define SHDISP_DBG_ERROR_LOG_SUMMARY_LINE15         (SHDISP_DBG_MODE_APPSBL_STR        \
                                                     ...
                                                     SHDISP_DBG_SUBCODE_POWERNG_STR    \
                                                     "   0  \n")
#define SHDISP_DBG_ERROR_LOG_SUMMARY_LINE16         (SHDISP_DBG_MODE_APPSBL_STR        \
                                                     SHDISP_DBG_TYPE_PANEL_STR         \
                                                     SHDISP_DBG_CODE_READERR_STR       \
                                                     SHDISP_DBG_SUBCODE_DISPONCHK_STR  \
                                                     "   0  \n")
#define SHDISP_DBG_ERROR_LOG_SUMMARY_LINE17         (SHDISP_DBG_MODE_APPSBL_STR        \
                                                     SHDISP_DBG_TYPE_PANEL_STR         \
                                                     SHDISP_DBG_CODE_ERRDET_STR        \
                                                     SHDISP_DBG_SUBCODE_DISPONCHK_STR  \
                                                     "   0  \n")
...
const static struct shdisp_dbg_error_code ErrorCodeIndex[] = {
    ...
    {SHDISP_DBG_MODE_APPSBL,        SHDISP_DBG_TYPE_PSALS,      SHDISP_DBG_CODE_RETRY_OVER,
                                                                                SHDISP_DBG_SUBCODE_POWER_ON_NG      },
    {SHDISP_DBG_MODE_APPSBL,        SHDISP_DBG_TYPE_PANEL,      SHDISP_DBG_CODE_READ_ERROR,
                                                                                SHDISP_DBG_SUBCODE_DISPON_CHK       },
    {SHDISP_DBG_MODE_APPSBL,        SHDISP_DBG_TYPE_PANEL,      SHDISP_DBG_CODE_ERROR_DETECT,
                                                                                SHDISP_DBG_SUBCODE_DISPON_CHK       },
...
/* ------------------------------------------------------------------------- */
int shdisp_dbg_API_ringbuffer_dump(char *buf)
{
    int l;
    int s;
    int len = 0;
...
static size_t shdisp_dbg_get_stacktrace(char *stackdumpbuf, size_t length)
{
   ...
   SHDISP_TRACE("ptr=0x%p len=%ld", stackdumpbuf, length);
    while (1) {
        int urc, rtn;
    ...
    SHDISP_TRACE("dump stuck trace size=%ld", writelen);
    return writelen;
}
...
static int shdisp_dbg_add_err_log_file(struct shdisp_dbg_error_code *code)
{
    ...
    if (IS_ERR_OR_NULL(fp)) {
        kfree(buf);
        SHDISP_ERR("Cannot open file: %s err=%p", SHDISP_DBG_DISPLAY_ERROR_FILE, fp);
        return SHDISP_RESULT_FAILURE;
    }
...
static int shdisp_dbg_summary_init_file(void)
{
    ...
    if (IS_ERR_OR_NULL(fp)) {
        SHDISP_ERR("Cannot create file: %s err=%p pid=%d tgid=%d comm=%s", SHDISP_DBG_DISPLAY_ERROR_FILE, fp,
...
static int shdisp_dbg_summary_countup_file(int idx)
{
    ...
    if (IS_ERR_OR_NULL(fp)) {
        kfree(buf);
        SHDISP_ERR("Cannot open file: %s err=%p pid=%d tgid=%d comm=%s", SHDISP_DBG_DISPLAY_ERROR_FILE, fp,
