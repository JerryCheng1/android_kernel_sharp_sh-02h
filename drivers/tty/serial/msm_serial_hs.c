...
#ifdef CONFIG_SERIAL_BLUETOOTH
#define PORT_ID_BT 0    /* ttyHS0 */
#endif /* CONFIG_SERIAL_BLUETOOTH */
...
static void msm_hs_unconfig_uart_gpios(struct uart_port *uport)
{
	...
	if (pdata) {
#ifdef CONFIG_SERIAL_BLUETOOTH
		if (uport->line == PORT_ID_BT) {
			if (gpio_is_valid(pdata->uart_rfr_gpio)) {
				int ret = 0;
				ret = gpio_request(pdata->uart_rfr_gpio,
								"UART_RFR_GPIO");
				MSM_HS_INFO("gpio_request(rfr_gpio=%d) call ret=%d\n"
								, pdata->uart_rfr_gpio, ret);
				if (unlikely(ret)) {
					MSM_HS_ERR("gpio request failed for:%d\n",
						pdata->uart_rfr_gpio);
				} else {
					MSM_HS_DBG("%s(): GPIO[48] RTS: HIGH\n", __func__);
					gpio_set_value(pdata->uart_rfr_gpio, 1);	/* RTS: HIGH */
				}
			}
		}
#endif /* CONFIG_SERIAL_BLUETOOTH */
			...
			if (pdata) {
				if (gpio_is_valid(pdata->uart_rfr_gpio)) {
					...
					if (unlikely(ret)) {
						...
						MSM_HS_DBG("%s(): GPIO[48] RTS: LOW\n", __func__);
						gpio_set_value(pdata->uart_rfr_gpio, 0);	/* RTS: LOW */
						gpio_free(pdata->uart_rfr_gpio);
					}
				}
			}
